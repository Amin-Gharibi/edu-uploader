import { createHeaderBanner, deleteHeaderBanner, editHeaderBanner, getHeaderBanners, getOneHeaderBanner } from "./funcs/headerBanners.js"
...
async function handlePanelHeaderBannerContent () {
    ...
    dynamicContentContainer.innerHTML = `
                <form class="add-focused-subject mt-10">
                    <div class="grid grid-cols-2 xl:grid-cols-3 gap-5 ">
                        <div class="h-max flex flex-col gap-y-2">
                ...
                <div class="all-header-banners-section-container max-w-[330px] xs:max-w-[390px] md:max-w-[490px] lg:max-w-[700px] xl:max-w-max mt-12 w-full ruonded p-5 customBoxShadow flex flex-col gap-y-6">
        ...
        return `
                                    <tr>
                                        ...
                                        <td>
                                            <button data-value="${headerBanner._id}" class="edit-btns text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded sm:rounded-lg py-1.5 px-5 text-sm focus:outline-none">
                                                ویرایش
                                            </button>
                                        </td>
                                        <td>
    ...
    const editHandler = async (btn) => {
        ...
        const data = await getOneHeaderBanner(btn.dataset.value.trim())

        if (data.ok) {
            const headerBanner = data.data.headerBanner

            const allHeaderBannersSectionContainer = document.querySelector('.all-header-banners-section-container')
            allHeaderBannersSectionContainer.classList.add('hidden')
            ...
            titleInput.value = headerBanner.title
            hrefInput.value = headerBanner.href
            coverInput.parentElement.insertAdjacentHTML("beforeend", `
                <a href="${BASE_URL}/headerBanners/${headerBanner.cover}" class="text-blue-500" target="_blank">لینک مشاهده</a>
            `)
            ...
            editSubmitBtn.addEventListener('click', async event => {
                ...
                const data = await editHeaderBanner(btn.dataset.value.trim(), titleInput.value.trim(), hrefInput.value.trim(), coverInput.files[0])
